...
(async () => {
	if (transport === 'stdio') {
		...
		if (!accessToken) {
			console.error('google-drive-mcp: GOOGLE_ACCESS_TOKEN required for stdio transport');
		...
		if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
			console.error('google-drive-mcp: GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET required for HTTP transport');
		...
		const protectedResourceMetadata: OAuthProtectedResourceMetadata = {
			...
			scopes_supported: DRIVE_SCOPES,
			resource_name: 'Google Drive MCP Server',
			resource_documentation: 'https://github.com/domdomegg/google-drive-mcp',
		};
		...
		const httpServer = app.listen(port, () => {
			console.error(`Google Drive MCP server running on ${baseUrl}/mcp`);
		});
